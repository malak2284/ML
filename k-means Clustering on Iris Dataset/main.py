...
# Wczytywanie pliku z dostosowanym separatorem i bez nagłówków
data = pd.read_csv('iris_training.txt', sep='\t', header=None)

# Podział danych na cechy i etykiety
input_data = data.iloc[:, :-1].apply(lambda x: x.map(lambda x: float(x.replace(',', '.')))).values
